...
func TestTrayIconCarriesAChipPerPhase(t *testing.T) {
	...
	seen := make(map[string][]core.Phase)
	...
	if len(seen) != distinct {
		grouped := make([][]core.Phase, 0, len(seen))
...
func TestTrayIconIsRenderedOnce(t *testing.T) {
	...
	first := icons.forPhase(core.PhaseConnected)
	if again := icons.forPhase(core.PhaseConnected); again != first {
...
func TestTrayIconFallsBackToTheArtwork(t *testing.T) {
	...
	if resource := icons.forPhase(core.PhaseConnected); resource != base {
