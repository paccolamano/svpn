...
func renderCases() map[string]core.Snapshot {
	return map[string]core.Snapshot{
		"connected": {
			Phase: core.PhaseConnected,
			Status: ipc.Status{
...
func TestWindowRendersEveryPhase(t *testing.T) {
	...
	for name, snapshot := range renderCases() {
		window := New(application, core.New(nil, nil, core.Options{}), branding)
