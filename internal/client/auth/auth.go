...
type Options struct {
	...
	Insecure bool
	// Debug traces the exchange with the gateway to stderr, which includes the
	// session cookie — a live credential. It is what svpn's --debug turns on,
	// and it is the reason that flag is not one to leave on.
	Debug bool
