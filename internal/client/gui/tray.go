...
func chipColor(phase state.Phase) (color.NRGBA, bool) {
	switch phase {
	...
	case state.PhaseDisconnected, state.PhaseNoDaemon:
...
type trayIcons struct {
	...
	source image.Image
	cache  map[state.Phase]fyne.Resource
}

func newTrayIcons(base fyne.Resource) *trayIcons {
	icons := &trayIcons{base: base, cache: make(map[state.Phase]fyne.Resource)}
...
func (t *trayIcons) forPhase(phase state.Phase) fyne.Resource {
...
func (t *trayIcons) render(phase state.Phase) fyne.Resource {
...
func (w *Window) installTray() {
	...
	w.setTrayIcon(state.PhaseUnknown)
...
// either gets to see.
func (w *Window) renderTray(snapshot state.Snapshot) {
	if w.tray == nil {
		return
	}
...
func (w *Window) setTrayIcon(phase state.Phase) {
