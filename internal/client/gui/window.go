...
func New(application fyne.App, controller *state.Controller, branding Branding) *Window {
	w := &Window{
		app:        application,
...
func (w *Window) Watch(updates <-chan state.Snapshot) {
...
func (w *Window) Render(snapshot state.Snapshot) {
	...
	connected := snapshot.Phase == state.PhaseConnected
...
// outstanding.
func (w *Window) renderLogin(snapshot state.Snapshot) {
...
func (w *Window) renderActions(snapshot state.Snapshot) {
	...
	if snapshot.Phase == state.PhaseAuthenticating {
		w.cancel.Show()
	} else {
		w.cancel.Hide()
	}
...
func statusColorName(phase state.Phase) fyne.ThemeColorName {
	switch phase {
	case state.PhaseConnected:
		return theme.ColorNameSuccess
	case state.PhaseNoDaemon:
		return theme.ColorNameError
	case state.PhaseConnecting, state.PhaseAuthenticating, state.PhaseDisconnecting:
