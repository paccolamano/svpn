...
func runUpdate(c *cobra.Command, args []string) error {
	...
	slog.Info("latest release", "version", latest.Tag, "installed", build.Version)

	upToDate := !release.Newer(latest.Tag, build.Version)
	if updateOpts.check {
		if upToDate {
	...
	if !release.IsRelease(build.Version) && !updateOpts.force {
		return errors.Errorf("this is a development build (%s), not a release; re-run with --force to replace it with %s", build.Version, latest.Tag)
	}
...
func handOver(ctx context.Context, dir string) error {
	...
	// An update installs what is installed. The archive carries a desktop
	// client whether or not this machine asked for one, so a headless install
	// has to say again that it does not want it.
	if !install.HasGUI(prefix) {
